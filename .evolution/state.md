...
- 检查：24 套件 headless（tests/runner/test_*.gd）+ verify_feedback 567 断言（无 lint）
...
- R86/R89/R90 2026-09-19：屏中大字自适应缩字号（toast/里程碑/连杀三处，长横幅不溢出）；法术圈伤害链探针实测有效（圈内直落——游戏内无伤=走位躲圈/护盾格挡，设计行为）；诺亚技能无环绕武器兜底实体僚机（SummonDrone 环绕+接触伤害，毒云同直结算通道）；verify 559/559
- R87/R88 2026-09-19：武器化身元素共鸣染色（dominant_element 稳定主色 + lerp 平滑，弹体配色同源）；难度门控（普通通关 best_wave≥final_wave 解锁困难/地狱，菜单置灰）+ 分难度槽位帽 3/4/5 + 金卡「武器槽+1」替换式低概率（独立 RNG 零扰动主序列，可越帽至 5）；pkg2/pkg5 槽位断言改地狱帽口径；verify 567/567 + 全电池零错
