...
**〔战地黑市+表现层 2026-08-30，art/daylight-pop 分支〕**（/goal 连续清单）——①M7 股市商店一期：金币经济接活（gold_drop 死数据首次接线：击杀 roll 掉账 + HUD 金币 pill）+ `scripts/ui/shop_ui.gd` 战地黑市（SHOP 波事件 → wave_director.shop_requested → GameLoop 切 LEVEL_UP 复用弹卡仲裁；行情系数 0.7~1.3 波次+槽位确定性漂移 ▲▼ 抄底提示；货架 = 词条×2（稀有度 roll+数值缩放同卡池）+ 治疗包 + 未持有遗物；刷新重掷）；②第 4 角色游侠·岚（血 52 攻 +10%，影袭瞬步 260px + 1s 无敌）；③养成二期：经验萃取 +8%/级×3、初始资金 +20/级×3；④Boss 出场横幅（弹入停留淡出）；⑤程序化音效 `scripts/gamefeel/sfx_bank.gd`（PCM 合成 9 音色，polyphony 4，高频事件 70ms 节流，事件总线全接）。验收 123 项全 PASS，回归 731/731 PASS。

**〔成就闭环+复活+词缀 2026-08-30，art/daylight-pop 分支〕**（/goal 连续清单）——①成就奖励结晶（11 条各 10~80💎，达成即发 + HUD toast 弹报，打通成就→养成闭环）；②养成三期：应急协议（复活 1 次/级×2，满血复活+2s 无敌，死亡路径 E-16 前插 _try_revive——测试无养成时仲裁口径不变）；③地图词缀（M2 二期）：霜冻之地（敌冰抗+20%）/魔血狂暴（敌速+10%）/迅捷之风（经验+10%）/毒性弥漫（敌血+8%），spawner 出生后差分应用 + 选图面板展示词缀行。验收 134 项全 PASS，回归 731/731 PASS。

**〔角色解锁链 2026-08-30，art/daylight-pop 分支〕**（用户反馈「角色通关大关解锁，每个大关一个，越后面越好」）——①CharacterTable 增 `unlock_map` 字段（空=初始）：薇拉←草原 / 磐←冰原（攻 -15%→-5% 增强）/ 岚←魔域 / 新增**演算者·零**←树海（血 65 攻 +15%，技能「时滞力场」全场静止 2.5s 无视免疫）/ 新增**腐化者·莽**←沼泽（血 58 攻 +12%，技能「毒沼绽放」全屏 150% 攻结算）；②Meta.is_character_unlocked 派生自 maps_cleared（零新增存档）；③set_character 解锁守卫（未解锁回落哨兵）；④选人面板锁定态（🔒 通关「图名」解锁，隐藏选用键）；⑤**真修**：时停原写 e.freeze_timer（Enemy 节点无此属性静默失败=时停无效）→ 改写 ElementalState.freeze_timer。验收 139 项全 PASS，回归 731/731 PASS。
